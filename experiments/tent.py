...
class Tent(nn.Module):
    ...
    """
    def __init__(self, model, optimizer, fishers=None, training_avg=None, training_var=None, steps=10, episodic=False):
        super().__init__()
        self.model = model
...
def collect_params(model, args):
    ...
    for nm, m in model.named_modules():
        ...
        if args.act_type != "relu" and args.act_flag:
...
def configure_model(model):
    ...
    for m in model.modules():
        ...
        if isinstance(m, FReLU):
            ...
            m.bn.running_mean = None
            m.bn.running_var = None
        if isinstance(m, nn.PReLU):
            m.requires_grad_(True)
